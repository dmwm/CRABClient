...
class SubCommand(ConfigCommand):
    ...
    def __init__(self, logger, cmdargs = []):
        ...
        if hasattr(self.options, 'task') and self.options.task:
            self.createCache()
            self.handleProxy(standalone=self.standalone,baseurl=self.getUrl(self.instance, resource='info'))
    ...
    def handleProxy(self, baseurl=None, standalone=False):
        ...
        if not self.options.skipProxy and self.initializeProxy:
            ...
            if 'rest' in alldns and alldns['rest']:
                delegateProxy(alldns['rest'], 'myproxy.cern.ch', self.proxyobj, self.logger, nokey=True)
            if 'services' in alldns:
                for serverdn in alldns['services']:
                    delegateProxy(serverdn, 'myproxy.cern.ch', self.proxyobj, self.logger, nokey=False)
        else:
